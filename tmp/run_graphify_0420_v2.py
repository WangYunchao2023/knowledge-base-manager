...
import json, time, sys, os, re, shutil
...
DRUG_TYPES = ["化学药物","中药","天然药物","生物制品","纳米药物","抗体偶联药物","脂质体药物",
    ...
    "仿制药","放射性药物","细胞与基因治疗药物","先进疗法","ADC","抗体偶联药物",
    "含毒性药材中药"]
TOX_STUDY_TYPES = ["急性毒性试验","长期毒性试验","重复给药毒性试验","生殖毒性试验","致癌试验",
    ...
    "主动全身过敏试验","被动皮肤过敏试验","一般药理学研究","抗肿瘤药物非临床评价"]
...
GUIDELINES = ["ICH S7A","ICH S7B","ICH S9","ICH S5(R3)","ICH S6(R1)","ICH S8","ICH S1A","ICH S1B",
    "ICH S1C","ICH S10","ICH E9","NMPA","CDE","EMA","WHO","药物致癌试验必要性","药物遗传毒性研究技术指导原则",
    "药物安全药理学研究技术指导原则","药物刺激性、过敏性和溶血性研究技术指导原则",
    "药物单次给药毒性研究技术指导原则","药物重复给药毒性研究技术指导原则",
    "药物毒代动力学研究技术指导原则","纳米药物非临床安全性研究技术指导原则",
    "抗体偶联药物非临床研究技术指导原则","脂质体药物非临床药代动力学研究技术指导原则",
    "药物免疫毒性非临床研究技术指导原则","药物临床试验期间安全性信息汇总分析和报告指导原则"]
...
print(f"  Top node: {top_by_degree[0]}")
...
try:
    ...
    with open(HTML_PATH, "w", encoding="utf-8") as f:
        f.write(html)
    shutil.copy(HTML_PATH, OUT / "anshang_graphify_semantic_cron_latest.html")
...
try:
    ...
    report_md = generate(G, clusters=communities,
...
semantic_full = {
    ...
    "metadata": {"total_files": len(all_to_process), "total_nodes": len(all_nodes),
