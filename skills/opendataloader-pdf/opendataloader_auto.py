    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=5) as resp:
            ...
            if any("qwen" in m.lower() and "vl" in m.lower() for m in models):
                print(f"[INFO] 检测到 qwen2.5vl 模型: {models}")
                return True
    except Exception:
        pass
    return False
...
def pdf_page_to_image(pdf_path: str, page_num: int, dpi: int = 200) -> bytes:
    """将 PDF 指定页转为图片字节流（使用 pypdfium2）"""
    import pypdfium2 as pdfium
    renderer = pdfium.PdfRenderer(pdf_path)
    page = renderer.get_page(page_num, scale=dpi / 72.0)
...
QWEN_OCR_PROMPT = """你是一个专业的文档 OCR 提取助手。请仔细识别这张 PDF 扫描页的内容，并按以下 JSON 格式返回：
...
{{
  ...
  "blocks": [
    {{
      "type": "text",
      "content": "<识别的文本内容，保持原有换行和段落结构>"
    }},
    {{
      "type": "table",
      "content": "<表格的 Markdown 格式，第一行为表头，用 | 分隔各列>\n<第二行为表头分隔线，如 |---|---|>\n<后续为数据行>"
    }}
  ]
}}
```
...
重要规则：
1. text 块：严格按照阅读顺序输出，保持自然段落的完整性
2. table 块：用标准 Markdown 表格格式（如 | col1 | col2 |）
3. 如果页面同时有文字和表格，都要识别并分开输出
4. 纯文字页面只需输出 text 块
5. 不要输出任何解释文字，只输出 JSON
6. 中文、英文、符号都要准确识别"""


def qwen_ocr_image(image_path: str, output_dir: str, lang: str = "zh,en") -> dict:
    """
    使用本地 qwen2.5vl 对单张图片进行 OCR
    支持: PNG, JPG, JPEG, WebP, BMP, TIFF
    返回: {{"success": bool, "json_path": str, "md_path": str}}
    """
    import urllib.request
    import base64
    ...
    print(f"[INFO] 使用 qwen2.5vl 对图片进行 OCR...")
    ...
    try:
        ...
        try:
            ...
            pass  # 已经是 PNG 或无法转换就用原始字节
        img_b64 = base64.b64encode(img_bytes).decode("ascii")
    except Exception as e:
        print(f"[错误] 读取图片失败: {e}", file=sys.stderr)
    ...
    payload = {
        ...
        "messages": [
            {
                "role": "user",
                "content": QWEN_OCR_PROMPT.replace('PDF 扫描页', '图片'),
                "images": [img_b64]
            }
        ],
        "stream": False,
    ...
    try:
        ...
        print(f"[错误] qwen2.5vl 请求失败: {e}", file=sys.stderr)
        return {"success": False}
    ...
    try:
        ...
        if start >= 0 and end > start:
            try:
                ...
                print(f"[错误] JSON 解析失败: {content[:200]}", file=sys.stderr)
                return {"success": False}
        else:
            print(f"[错误] 无法提取 JSON", file=sys.stderr)
            return {"success": False}
    ...
    print(f"[成功] 图片 OCR 完成: {len(all_elements)} 个元素")
...
def qwen_ocr_pdf(pdf_path: str, output_dir: str, lang: str = "zh,en") -> dict:
    ...
    print(f"[INFO] 使用 qwen2.5vl 进行 OCR 识别...")

    try:
        import pypdfium2 as pdfium
    except ImportError:
        print("[错误] pypdfium2 未安装，请运行: pip install pypdfium2", file=sys.stderr)
        return {"success": False}

    try:
        renderer = pdfium.PdfRenderer(pdf_path)
        total_pages = len(renderer)
    ...
    for page_idx in range(total_pages):
        ...
        try:
            # 渲染页面为图片
            page = renderer.get_page(page_idx, scale=200 / 72.0)
        ...
        try:
            ...
            print(f"[警告] 第 {page_num} 页 OCR 请求失败: {e}", file=sys.stderr)
            continue
        ...
        try:
            ...
            if start >= 0 and end > start:
                try:
                    ...
                    print(f"[警告] 第 {page_num} 页 JSON 解析失败，跳过")
                    continue
            else:
                print(f"[警告] 第 {page_num} 页无法提取 JSON，跳过")
                continue
        ...
        print(f"[INFO] 第 {page_num}/{total_pages} 页完成" + (
            f" → 含 {len([b for b in page_data.get('blocks', []) if b.get('type')=='text'])} 段文字, "
            f"{len([b for b in page_data.get('blocks', []) if b.get('type')=='table'])} 张表格" if page_data.get('blocks') else ""
        ))
    ...
    print(f"[成功] qwen2.5vl OCR 完成: {len(all_elements)} 个元素 ({total_paras} 段, {total_tables} 张表格)")
    return {"success": True, "json_path": str(json_path), "md_path": str(md_path), "total_pages": total_pages}
...
def run_convert(pdf_path: str, output_dir: str,
    ...
    # ---- 图片文档：qwen2.5vl OCR ----
    ext_lower = pdf_path.lower()
    if (ext_lower.endswith(".png") or ext_lower.endswith(".jpg") or
            ext_lower.endswith(".jpeg") or ext_lower.endswith(".webp") or
            ext_lower.endswith(".bmp") or ext_lower.endswith(".tiff")):
        print(f"[INFO] 检测到图片文档 → qwen2.5vl OCR 提取")
        if not check_qwen_available():
            print("[错误] qwen2.5vl 不可用，请确保 Ollama 运行中", file=sys.stderr)
            return {"success": False, "mode_used": "image-qwen-unavailable"}
        ocr_result = qwen_ocr_image(pdf_path, output_dir, lang="zh,en")
        if ocr_result.get("success"):
            return {"success": True, "mode_used": "qwen2.5vl-ocr",
                    "files_created": [ocr_result["json_path"], ocr_result["md_path"]]}
        return {"success": False, "mode_used": "image-failed"}
    ...
    # 强制 qwen2.5vl OCR 模式
    if force_qwen and check_qwen_available():
        print("[INFO] 强制使用 qwen2.5vl OCR 模式", file=sys.stderr)
        ocr_lang = detection.get("lang", "auto") or "zh,en"
        if ocr_lang == "auto":
            ocr_lang = "zh,en"
        ocr_result = qwen_ocr_pdf(
            pdf_path=pdf_path,
            output_dir=output_dir,
            lang=ocr_lang
        )
        if ocr_result.get("success"):
            ...
            print("[错误] qwen2.5vl OCR 失败，尝试其他模式...", file=sys.stderr)
    ...
    if detection["type"] != "digital" and not skip_server:
        ...
        if not server_ok:
            # 尝试 qwen2.5vl OCR（备选方案）
            print("[INFO] Hybrid server 不可用，尝试 qwen2.5vl OCR...", file=sys.stderr)
            if check_qwen_available():
                print("[INFO] 使用 qwen2.5vl 进行扫描 PDF OCR", file=sys.stderr)
                ocr_result = qwen_ocr_pdf(
                    pdf_path=pdf_path,
                    output_dir=output_dir,
                    lang=ocr_lang
                ...
                if ocr_result.get("success"):
                    ...
                    print("[错误] qwen2.5vl OCR 失败，尝试降级到 Fast 模式...", file=sys.stderr)
            else:
                print("[错误] Hybrid server 启动失败，且未检测到 qwen2.5vl，尝试降级到 Fast 模式...", file=sys.stderr)
...
def main():
    ...
    parser.add_argument("--force-qwen", action="store_true",
                        help="强制使用 qwen2.5vl 做 OCR（扫描PDF和图片推荐）")
    parser.add_argument("--image-dpi", type=int, default=200,
                        help="PDF/图片渲染 DPI（默认 200）")
    ...
    if args.detect_only:
        ...
        elif ext.endswith(".png") or ext.endswith(".jpg") or ext.endswith(".jpeg") or ext.endswith(".webp") or ext.endswith(".bmp") or ext.endswith(".tiff"):
            info = {"type": "image", "format": ext}
        else:
