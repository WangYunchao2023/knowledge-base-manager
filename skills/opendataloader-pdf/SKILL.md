...
**OCR 引擎优先级（扫描 PDF）**：
> qwen2.5vl（VLM，表格理解最强）> Hybrid/EasyOCR（深度学习 OCR）> Fast（本地，保底）

**准确率 Benchmark**：
| 引擎 | 总体准确率 | 表格 | 速度 |
|------|-----------|------|------|
| qwen2.5vl (VLM) | **最高** | **最强** | 慢（~数秒/页）|
...
# 3. 强制跳过 Hybrid，直接用 qwen2.5vl + Fast（不用 EasyOCR）
...
3. 有文字页 < 20%（扫描 PDF）→ **qwen2.5vl 优先**（VLM，表格理解最强）
   - **qwen2.5vl 成功** → 直接完成
   - **qwen2.5vl 失败** → **降级到 Hybrid/EasyOCR**（深度学习 OCR）
...
| 每个元素带边界框（bbox）| ✅ | 本地 |
| 简单表格提取（边框表格）| ✅ | 本地 |
| 复杂/无边框表格 | ✅ | qwen2.5vl（主）/ Hybrid（备）|
| 标题层级检测 | ✅ | 本地 |
| 列表检测（有序/无序/嵌套）| ✅ | 本地 |
| 图片提取 + 坐标 | ✅ | 本地 |
| AI 图表描述 | ✅ | Hybrid |
| 扫描 PDF OCR（80+ 语言）| ✅ | qwen2.5vl（主）/ Hybrid（备）|
| 公式提取（LaTeX）| ✅ | Hybrid |
| Tagged PDF 结构提取 | ✅ | 本地 |
| AI 安全过滤（防注入）| ✅ | 本地 |
| 页眉/页脚/水印过滤 | ✅ | 本地 |
...
| 扫描 PDF 未提取 | `opendataloader_auto.py` 自动优先用 qwen2.5vl，失败后自动降级 Hybrid |
| 混合 PDF（扫描+文字）| `opendataloader_auto.py` 自动走 qwen → Hybrid → Fast 三级降级 |
