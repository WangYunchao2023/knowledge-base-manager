...
description: 文档解析工具（统一处理 PDF / Word / Excel / 图片）。新增本地 qwen2.5vl OCR 引擎，支持图片 OCR、扫描版 PDF 处理。Word 采用「docx内容 + PDF位置」合并；Excel 采用「openpyxl结构化数据 + Sheet名标记」。统一输出 JSON（含 location/headers/data_rows）和 Markdown，方便 AI 读取、分析、汇总与自动撰写。
---
...
- ✅ 边界框（bbox）定位每个元素
- ✅ 表格提取（简单/复杂无边框）
- ✅ **qwen2.5vl OCR（本地 VLM，扫描 PDF 专用）**
- ✅ OCR（80+语言，扫描 PDF，Hybrid 模式）
- ✅ 公式提取（LaTeX）
- ✅ 图表 AI 描述
...
```bash
# 1. 自动检测并转换（智能选择 Fast / Hybrid / qwen 模式）
...
# 3. 强制使用 qwen2.5vl OCR（扫描 PDF 推荐用法）
python3 opendataloader_auto.py document.pdf -o output/ --force-qwen
...
3. 有文字页 < 20% → 尝试 Hybrid + OCR 模式
   - **Hybrid 成功** → AI 增强 OCR
   - **Hybrid 失败** → **自动降级到 qwen2.5vl OCR**（如果本地 qwen2.5vl 可用）
   - **均不可用** → 降级到 Fast 模式
4. 混合文档（20%~80%）→ **Hybrid + force-ocr + full 模式**，全部页面走 AI
5. 自动检测语言（中文/英文），用于 OCR 配置

**扫描 PDF 推荐用法**：使用 `--force-qwen` 强制调用本地 qwen2.5vl（8.3B，支持视觉，OCR + 表格理解能力强）：
```bash
python3 opendataloader_auto.py scanned_report.pdf -o output/ --force-qwen
```

**qwen2.5vl OCR 特点**：
- 📷 每页转为图片，通过 Ollama API 调用本地 qwen2.5vl
- 🧠 VLM 理解能力强，表格结构重建效果优于传统 OCR
- 🇨🇳 中文识别准确率高
- 🔒 完全本地运行，不走外网
...
| 格式 | 处理方式 | 输出格式 | 位置追溯 |
|------|---------|---------|---------|
| PDF（数字/扫描/混合）| 自动路由（Fast / Hybrid / qwen2.5vl）| Markdown + JSON | ✅ 完整（页码+bbox） |
...
| 图片（PNG/JPG/WebP/BMP/TIFF）| **qwen2.5vl OCR**（本地 VLM）| Markdown + JSON | ⚠️ 单页（无多页结构） |
...
- Python 3.10+
- Python 包：`pypdf`（自动检测用）、`pypdfium2`（PDF渲染）、`opendataloader-pdf`
- **Ollama + qwen2.5vl**（图片/扫描 PDF OCR 用）
...
| **qwen2.5vl（本地 VLM）** | 扫描 PDF / 图片（推荐 `--force-qwen`） | ❌ 不需要 | 慢（~3-5s/页）| ⭐⭐⭐⭐⭐ |
